...
function parseAllowed(raw: string, suffixes: string[]): URL | null {
  ...
  const ok = suffixes.some(
    (s) => host === s.replace(/^\./, "") || host.endsWith(s),
  );
  return ok ? parsed : null;
}

async function followWithAllowlist(
  initial: URL,
  signal: AbortSignal,
): Promise<Response> {
  let current = initial;
  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    ...
    const next = parseAllowed(
      new URL(location, current).toString(),
      REDIRECT_ALLOWED_SUFFIXES,
    );
...
export async function GET(request: Request) {
  ...
  if (!target)
    return new Response(JSON.stringify({ error: "Disallowed url" }), {
      status: 400,
    });
  ...
  try {
    ...
    return new Response(
      streamWithCleanup(upstream.body, () => clearTimeout(timeout)),
      {
        status: 200,
        headers,
      },

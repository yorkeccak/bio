...
if (isProduction) {
  try {
    ...
    console.warn(
      "[PDF Generation] @sparticuz/chromium not available, using local puppeteer",
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      "[PDF Generation] Starting PDF generation for session:",
      sessionId,
    );
    ...
    console.log(
      "[PDF Generation] Extracted content length:",
      markdownContent.length,
    );
    console.log("[PDF Generation] Found", citations.length, "citations");
    console.log(
      "[PDF Generation] Total processing time:",
    ...
    console.log(
      "[PDF Generation] Found",
      csvIds.length,
      "CSV tables to render",
    );

    // Step 4: Preprocess markdown
    const processedMarkdown = preprocessMarkdownText(
      cleanBiomedicalText(markdownContent),
    );
    ...
    try {
      ...
      if (chartIds.length > 0) {
        ...
        const chartPromises = chartIds.map((chartId) =>
          renderChartAsImage(browser, chartId),
        );
        ...
        console.log(
          "[PDF Generation] Charts rendered:",
          chartImagesMap.size,
          "/",
          chartIds.length,
        );
...
async function renderChartAsImage(
  ...
  const parsedChartData =
    typeof chartData.chart_data === "string"
      ? JSON.parse(chartData.chart_data)
      : chartData.chart_data;

  console.log(
    "[PDF Generation] Chart data parsed:",
    JSON.stringify(parsedChartData, null, 2).slice(0, 500),
  );
  ...
  try {
    ...
    console.log(
      "[PDF Generation] Chart rendered successfully:",
      chartId,
      "Size:",
      base64.length,
    );
...
function generateSVGChart(chartData: any, colors: string[]): string {
  ...
  if (chartType === "bar") {
    const barWidth =
      chartWidth / (xLabels.length * dataSeries.length + xLabels.length);
...
function getToolType(
  ...
  if (name.includes("literature") || name.includes("biomedical"))
    return "literature";

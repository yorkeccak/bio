...
import { DOMAINS, LIFE_SCIENCES_LENSES } from "@/lib/domains";
import { getExample, isExampleReady } from "@/lib/example-reports/registry";
import { ExampleReportDrawer } from "@/components/research/example-report-drawer";

/** One glyph per lens — matches the reports browser, stays monochrome. */
...
export function HomeWorkflows() {
  ...
  const examples = LIFE_SCIENCES_LENSES.map((v) => getExample(v)).filter(isExampleReady);
  ...
  return (
    ...
    >
      ...
      {/* Lens pills → deep-link into the reports browser */}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {examples.map((ex) => {
          const Icon = DOMAIN_ICON[ex.domainId] ?? Microscope;
          return (
            <button
              key={ex.domainId}

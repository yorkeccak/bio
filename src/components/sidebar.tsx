...
import { apiReportHistory, apiDeleteReport } from "@/lib/report-client";
...
function ReportStatusDot({ status }: { status: string }) {
  ...
  return (
    <span className="relative flex h-2.5 w-2.5 flex-shrink-0">
      {!isTerminal(status) && (
        <span
          className={`absolute inline-flex h-full w-full rounded-full opacity-60 animate-ping ${color}`}
        />
      )}
      <span
...
export function Sidebar({
  ...
  const unseenReports = unseenCompletedCount(reports);
                ...
                >
                  <svg
                    className={`w-6 h-6 transition-colors ${
                      alwaysOpen ? "text-foreground" : "text-muted-foreground"
                    }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"

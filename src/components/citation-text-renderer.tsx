...
const parseGroupedCitations = (
  text: string,
): {
  segments: Array<{
    type: "text" | "citation-group";
    content: string;
    citations?: string[];
  }>;
} => {
...
const createMarkdownComponents = (citations: CitationMap) => ({
            ...
            if (segment.type === "citation-group" && segment.citations) {
              return (
                <GroupedCitationBadge
                  key={idx}
                  citationKeys={segment.citations}
                  citations={citations}
                />
              );
            }
  ...
  math: ({ children }: any) => {
    ...
    try {
      ...
      return (
        <span dangerouslySetInnerHTML={{ __html: html }} className="katex-math" />
      );
    } catch {
      return (
...
export const CitationTextRenderer = React.memo(
  ({ text, citations, className = "" }: CitationTextRendererProps) => {
    ...
    const hasCitationGroups =
      parsedSegments &&
      parsedSegments.segments.some((s) => s.type === "citation-group");
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      Object.keys(prevProps.citations).length ===
        Object.keys(nextProps.citations).length &&

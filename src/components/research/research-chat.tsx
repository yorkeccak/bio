...
import { requestNotifyPermission } from "@/components/research/report-notify";
import { ReportView } from "@/components/research/report-view";
import { ErrorNote } from "@/components/research/error-note";
...
const isAuthError = (message: string): boolean =>
  /AUTH_REQUIRED|session expired|sign in|unauthori/i.test(message);
...
const DELIVERABLE_FORMATS: {
...
}[] = [
  {
    ...
    sample: "e.g. Trial tracker: NCT ID, sponsor, phase, primary endpoint, readout date",
  },
  {
    type: "pptx",
    label: "PowerPoint (.pptx)",
    icon: "/assets/filetypes/powerpoint.svg",
    sample: "e.g. 10-slide landscape deck: mechanism, assets, trials, risks, outlook",
  },
  {
    type: "docx",
    label: "Word (.docx)",
    icon: "/assets/filetypes/word.svg",
    sample: "e.g. Full memo with exec summary, evidence review and appendix",
  },
  {
    type: "csv",
    label: "CSV (.csv)",
    icon: "/assets/filetypes/csv.svg",
    sample: "e.g. Asset table: drug, target, indication, phase, sponsor",
  },
  {
    type: "pdf",
    label: "PDF (.pdf)",
  ...
  const addDeliverable = () =>
    setTools((prev) =>
      prev.deliverables.length >= MAX_DELIVERABLES
        ...
        : {
            ...prev,
            deliverables: [...prev.deliverables, { type: "xlsx", description: "" }],
          },
    );
    ...
    try {
      ...
      if (suggestedForRef.current !== q) {
        ...
        if (unclaimed) {
          ...
          const late = await apiSuggestDeliverables(
            q,
          ...
          if (late.length > 0) {
            ...
            setDeliverablesOpen(true);
            setLaunchNote(
              `Adding ${describeDeliverables(deliverables)} — starting research…`,
            );
          } else {
            setLaunchNote("Starting research…");
          }
      ...
      // A signed-out visitor gets the sign-in sheet, not a dead-end error.
      if (isAuthError(message)) setShowAuthModal(true);

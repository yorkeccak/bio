...
const MIGRATION_STORAGE_KEY = 'bio-migration-banner-dismissed';
const DISCORD_STORAGE_KEY = 'bio-discord-banner-dismissed';
...
export function MigrationBanner() {
  ...
  if (showMigration) {
    return (
      <div ref={bannerRef} className="fixed top-0 left-0 right-0 z-50 hidden md:block animate-in slide-in-from-top duration-300">
        <div className="bg-muted/80 backdrop-blur-sm border-b border-border/50">
          <div className="max-w-4xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
            <p className="text-sm text-muted-foreground">
              ...
              If you had an account, sign in with Valyu to access your reports.
            </p>
            <button
  ...
  if (showDiscord) {
    return (
      <div className="fixed top-3 left-3 z-50 hidden md:block animate-in slide-in-from-left duration-300">

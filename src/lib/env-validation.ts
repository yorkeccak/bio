...
export function validatePaymentEnvironment(): EnvValidationResult {
  ...
  if (isSelfHostedMode) {
    ...
    if (
      ...
      warnings.push(
        "Neither VALYU_API_KEY nor Valyu OAuth credentials are set - searches will fail",
      );
    }
    ...
    if (
      ...
      warnings.push(
        "No LLM provider configured - set OPENAI_API_KEY, OLLAMA_BASE_URL, or LMSTUDIO_BASE_URL",
      );
    }
  ...
  if (
    ...
    !process.env.NEXT_PUBLIC_VALYU_SUPABASE_URL.startsWith("https://")
  ) {
